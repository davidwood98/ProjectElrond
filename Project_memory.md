...
design** — it hard-depends on the `recognized_lines` cache (FA-24b / ai-semantic-layer-design.md)
which doesn't exist yet; per the spec, Phases 1–2 only, and no new ML Kit recognition pass was
added. **DB is now v18** (`MIGRATION_16_17` notebook_links, `MIGRATION_17_18` tags/notebook_tags).
**493 app + 24 aibackend JVM/Robolectric tests pass** (0 failures; was 472+24, and `./gradlew test`
is green in both variants); `:app:assembleDebug` + `:app:assembleDebugAndroidTest` build on the WSL
Linux SDK. The Compose/gesture surfaces are **device-verify pending** (list at the end). Key spec
correction discovered up front: the spec's FKs said `note_pages`, but since FA-20 notebooks and
pages are separate tables — link targets and tag membership key on **`notebooks(id)`**, while
navigation stays page-based (`onOpenNote(pageId)`).

- **Phase 1 — notebook cross-linking (v17).** On-canvas **link boxes** referencing another
  notebook: `notebook_links(id PK, sourcePageId FK→note_pages CASCADE, targetNotebookId
  FK→notebooks SET NULL, targetPageId reserved/unused, x/y/widthPx/heightPx, linkText cached
  title, createdAt)` — indices on both FK columns. Created via the toolbar **Add(+)** menu (the old
  Import placeholder is now a general add-in menu: **Link notebook** live, **Add page** a reserved
  disabled slot) → **Quick Nav in pick mode**; Quick Nav also gained a **search field** (pure
  `domain/QuickNavSearch` — notebook title OR owning subject name, case-insensitive), a standalone
  improvement present in both modes. The box (`ui/NotebookLinkView`, a **separate composable** —
  `AiInkNoteView` gained zero branches) reuses the passive pattern: no pointer input (pen writes
  over it), page-space offset + `graphicsLayer` zoom, `onMeasured` size reporting; rendered after
  the AI notes so links sit on top (hit-test checks `linkAt` before `aiNoteAt` to match — an
  ordering rule, not a z-order system). **Tap opens the target** (novel tap-to-open gesture in
  `InkCanvas`: a clean UP with no 800ms hold fired and no movement past the slop cancels the
  nascent stroke and opens; scroll-mode finger tap opens too) — resolved via
  `NoteRepository.lastViewedPageId(notebookId)` (live rows only, so a deleted last-viewed page
  falls back to the cover; an EMPTY notebook shows the transient pill instead of silently failing)
  and emitted on a dedicated **`openLinkEvents`** SharedFlow → `onOpenNote` (deliberately NOT
  `pageTurnEvents`, which is FA-20's intra-notebook navigation). **Hold selects** through the
  unified `SelectionState` — new third id-set **`linkIds`** (+`hasLink`/`isSingleLink`;
  `isSingleAiNote`/`canGroup` now require `linkIds.isEmpty()`) threaded through
  lasso/transform/duplicate/delete/copy/cut/paste/clearPage, and **`HistorySnapshot` gained
  `links` from day one** (the FA-21 lesson applied proactively — undo/redo covers links in every
  mutation). Persistence mirrors ai_notes: third debounced autosave collector + `onCleared` flush
  via `NotebookLinkRepository.replaceForPage`. **`NotebookLink.createdAt` round-trips verbatim
  (never re-stamped at persist time)** — it drives Backlinks ordering; do not "fix" it for
  consistency with `AiInkNote`. **Broken link** (target deleted → FK SET NULL): renders greyed
  "Reference not found", tap is dead (FA-5 dead-link rule), hold opens a **screen-centred**
  AlertDialog with exactly **Redefine** (re-runs the picker in redefine mode, replacing
  target+label in place) and **Delete** — centred by design, so the CLAUDE.md edge-clamp rule
  can't be violated. **Backlinks**: new ⋮ More-menu entry → `BacklinksDialog` listing every
  notebook linking here (DAO JOIN through `note_pages` for the source notebook; titles resolved in
  the VM via `observeNotebookSummaries` so the title-fallback rule isn't duplicated), rows open
  the source page directly via the `onOpenNote` callback (never the SharedFlow).
- **Phase 2 — notebook tagging (v18).** Flat many-to-many labels, independent of Subjects
  (**auto-tag-on-subject explicitly rejected per spec — not implemented**): `tags(id PK, name
  UNIQUE, colorArgb)` + `notebook_tags(notebookId, tagId, composite PK, both FKs CASCADE,
  Index(tagId))`. The legacy `note_pages.tags List<String>` column is unrelated and untouched.
  **Colour is deterministic per NAME** (`domain/TagColor.forName` → `SubjectPalette.argb(hash)`),
  resolved once at creation and **stored** — never recomputed, so a palette change can't reflow
  existing tags. One **`TagRepository`** (get-or-create by unique trimmed name, idempotent assign,
  remove) + one **`TagPickerDialog`** serve BOTH surfaces — the Library card ⋮ menu's new **Tags**
  entry and the editor header's tag row **+** — so they can't drift; the picker **toggles**
  membership (assigned rows remove immediately — a modal list is an explicit action; the undo
  window belongs to the always-visible row). **Header tag row** (`ui/TagComponents.kt`): a **hard
  fixed-width** (220dp) region right of the created date — never grows/shrinks/shifts regardless
  of tag count or title length (title keeps its own 320dp cap + ellipsis); overflow **scrolls
  horizontally with fade-out gradients** on each edge hiding content; each pill truncates
  independently at 96dp. **Pill gesture** (pure `domain/TagTapResolver`, 300ms window matching the
  app convention, uniform regardless of truncation): tap 1 → preview (expanded pill, full name);
  tap 2 same pill within 300ms → grey-out in place; `TagViewModel.beginUntag` runs a **2s
  cancellable window** (one Job per pill key) — tapping the greyed pill cancels with **no DB write
  ever having happened**; an uncancelled window removes the membership and the pill **collapses
  its width** (ghost map + `AnimatedVisibility(shrinkHorizontally)`, never a jump-cut). Screen
  dispose **flushes** pending removals via a companion `flushScope` (the CanvasViewModel
  autosave-flush precedent) — the user committed on the second tap. `TagViewModel` is per-screen
  by design (data converges via Room invalidation; the window is screen-local UI state — do not
  centralise it). `CanvasViewModel` exposes `notebookIdFlow` for the header wiring.
- New/updated tests: `StrokeSelectionTest` (+linkIds helpers/guards), `NotebookLinkTest`
  (defaultLinkPosition centre/zoom/clamp/zero-canvas), `QuickNavSearchTest`,
  `NotebookLinkRepositoryTest` (round-trip + ordering, **target-delete → SET NULL broken state**,
  source-page cascade, backlinks JOIN newest-first, createdAt survives unrelated rewrites,
  **lastViewedPageId fallback + empty→null**), `CanvasViewModelLinkTest` (create/tap incl.
  fallback + empty-notebook transient message + selected/broken no-ops, hold select vs broken
  menu, redefine/delete, lasso+move/scale/duplicate/copy-cut-paste/clearPage **each one undoable
  step**, observeBacklinks title resolution + "Untitled" fallback), `TagColorTest` (determinism,
  negative hash, empty), `TagTapResolverTest` (full transition table incl. the exact-300ms
  boundary), `TagRepositoryTest` (dedupe, many-to-many, idempotent assign, both cascade
  directions), `TagViewModelTest` (cancel-before-2s = no write, elapse = one write, idempotent
  while pending, independent per-pill windows, scoped pending ids), `ElrondMigrationTest` (chain →
  **v18**, per-migration table/index tests + a post-migration FK-cascade assert on the migrated
  schema); `LibraryScreenTest` updated to supply the `TagViewModel`.
- **Device-verify pending (Galaxy Tab S):** link-box **tap-vs-hold-vs-write-over** disambiguation
  (the first tap-to-open gesture in the canvas — most novel piece), broken-link flow, Quick Nav
  search + pick mode, Backlinks, link boxes in mixed lasso selections at zoom; tag row layout
